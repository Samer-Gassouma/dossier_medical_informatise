...
export default function ChartingTooth() {
  const router = useRouter();
  const patientID  = JSON.parse(router.query.results) ;
  useEffect(() => {
    fetch(`/api/tooth-status?p_id=${patientID}`)
      .then((res) => res.json())
      .then((data) => {
        ToothColor(data);
      }
  ...
  const ToothColor = (data) => {
    data.map((tooth) => {
      let toothId = document.getElementById("Tooth" + tooth.tooth_number);
      if (tooth.tooth_condition === "caries") {
        toothId.setAttribute("fill", "red");
      } else if (tooth.tooth_condition === "filling") {
        toothId.setAttribute("fill", "yellow");
      } else if (tooth.tooth_condition === "Missing") {
        toothId.setAttribute("fill", "blue");
      } else if (tooth.tooth_condition === "implant") {
        toothId.setAttribute("fill", "purple");
      } else if (tooth.tooth_condition === "Healthy") {
        toothId.setAttribute("fill", "green");
      } else {
        toothId.setAttribute("fill", "white");
      }
    });
  };

  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 450 700">
      <g id="toothLabels">
        <text
          id="lbl32"
